...
CONT_ACTION_DIM = int(getattr(para, "CONT_ACTION_DIM", 1))
SPLIT_SAT_PAIRS = list(combinations(range(S), 2)) if getattr(para, "ENABLE_TWO_SAT_SPLIT", 0) else []
SPLIT_ACTION_START = N + S + 1
ACTION_DIM = N + S + 1 + len(SPLIT_SAT_PAIRS)
...
TOPO_ALL = np.ones(ACTION_DIM, dtype=bool)
TOPO_LOCAL_ONLY = np.zeros(ACTION_DIM, dtype=bool)
TOPO_LOCAL_ONLY[0] = True
TOPO_NO_SAT = np.zeros(ACTION_DIM, dtype=bool)
TOPO_NO_SAT[: N + 1] = True
...
def apply_topology_mask(env_mask: np.ndarray, topo_mask: np.ndarray) -> np.ndarray:
    ...
    """
    if len(topo_mask) != len(env_mask):
        fixed_mask = np.zeros_like(env_mask, dtype=bool)
        copy_len = min(len(topo_mask), len(env_mask))
        fixed_mask[:copy_len] = topo_mask[:copy_len]
        topo_mask = fixed_mask
...
def summarize_debug(debug_info):
    ...
    split_steps = max(float(debug_info.get("split_metric_steps", 0.0)), 1.0)
    split_to = float(debug_info.get("split_timeout_actions", 0.0))
    split_dead = float(debug_info.get("split_deadline_success_actions", 0.0))
    ...
    return {
        ...
        "split_deadline_success_rate": split_dead / max(split_act, 1.0),
        "split_timeout_rate": split_to / max(split_act, 1.0),
        "avg_split_delay_over_gamma": float(debug_info.get("split_delay_over_gamma_sum", 0.0)) / max(split_to, 1.0),
        "avg_split_ratio": float(debug_info.get("split_ratio_sum", 0.0)) / split_steps,
        ...
        "sat_actions_count": sat_act,
        "split_actions": split_act,
    }
...
def _estimate_optimistic_delay(md, action_idx: int, bs_list, sat_list) -> float:
    ...
    if N + 1 <= action_idx <= N + S:
        ...
        if not md.is_sat_visible(sat):
            return float("inf")
        rate_sat = para.SAT_BW * math.log2(1.0 + 100.0)
    ...
    if pair_idx < 0 or pair_idx >= len(SPLIT_SAT_PAIRS):
        return float("inf")
    sat_a_idx, sat_b_idx = SPLIT_SAT_PAIRS[pair_idx]
    ...
    rate_sat = para.SAT_BW * math.log2(1.0 + 100.0)
    delay_parts = []
    for sat, ratio in ((sat_a, 0.5), (sat_b, 0.5)):
        part_bits = max(B * ratio, 1.0)
        t_prop = md.sat_distance(sat) / para.LIGHT_SPEED
        t_tran = part_bits / rate_sat
        t_comp = part_bits * C / para.SAT_F_MAX
...
def _policy_local_only(obs, md, mask, bs_list, sat_list):
    ...
    return 0, np.ones(CONT_ACTION_DIM, dtype=np.float32)
...
def _policy_random_equal(obs, md, mask, bs_list, sat_list):
    ...
    return disc, np.full(CONT_ACTION_DIM, 0.5, dtype=np.float32)
...
def _policy_greedy_delay(obs, md, mask, bs_list, sat_list):
    ...
    return best_action, np.ones(CONT_ACTION_DIM, dtype=np.float32)
